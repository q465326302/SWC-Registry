...
## 关系
[CWE-696: 不正确的行为顺序](https://cwe.mitre.org/data/definitions/696.html)

## 描述
Solidity支持多重继承，意味着一个合约可以继承多个合约。多重继承引入了称为[钻石问题](https://en.wikipedia.org/wiki/Multiple_inheritance#The_diamond_problem)的歧义：如果两个或更多的基础合约定义了相同的函数，那么在子合约中应该调用哪一个？Solidity通过使用[逆C3线性化](https://en.wikipedia.org/wiki/C3_linearization)来处理这种歧义，它在基础合约之间设置了优先级。

这样，基础合约具有不同的优先级，因此继承顺序很重要。忽视继承顺序可能导致意外行为。

## 修复
当继承多个合约时，特别是如果它们具有相同的函数，开发人员应该仔细指定正确的继承顺序。经验法则是从更 /一般/ 到更 /具体/ 继承合约。
