...
## 描述
当合同严格假设特定的以太币余额时，合同可能会出现错误行为。可以通过使用selfdestruct或通过向账户进行挖矿来强制向合同发送以太币（而不触发其回退函数）。在最坏的情况下，这可能导致DOS条件，使合同无法使用。

## 修复措施
避免在合同中对以太币余额进行严格的相等性检查。

## 参考资料
[Consensys 最佳实践：强制喂食](https://consensys.github.io/smart-contract-best-practices/attacks/force-feeding/)
[Sigmaprime：意外的以太币](https://blog.sigmaprime.io/solidity-security.html#ether)
