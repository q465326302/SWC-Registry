...
## 关系
[CWE-667: 锁定不当](https://cwe.mitre.org/data/definitions/667.html)

## 描述
当合约严格假设特定的以太余额时，可能会导致合约出现错误行为。可以通过使用selfdestruct函数或通过向账户进行挖矿来强制向合约发送以太（而不触发其回退函数）。在最糟糕的情况下，这可能导致出现DOS服务的情况，使合约无法使用。
