...
## 描述
由于缺乏或不充分的访问控制，恶意方可以自毁合约。

## 修复措施
如果没有绝对的必要，请考虑移除自毁功能。如果存在有效的使用案例，建议实施多签名方案，以便多个参与方必须批准自毁操作。

## 参考资料
* [Parity "我不小心杀死了它" 漏洞](https://www.parity.io/a-postmortem-on-the-parity-multi-sig-library-self-destruct/)
...
contract WalletLibrary is WalletEvents {
  ...
  // 多重签名函数修饰符：操作必须具有内在哈希，以便后续的尝试可以被视为相同的底层操作，并因此计为确认。
  ...
  // 构造函数给出了执行受保护的“onlymanyowners”交易所需的签名数量，以及能够确认它们的地址选择。
  ...
  // 通过哈希来确认交易。我们使用先前的交易映射表m_txs来确定提供的哈希的交易内容。
...
## 验证
部署suicide_multitx_feasible.sol中的合约，调用init（）函数修改initialized，再调用run（）函数会发出自毁指令，查看count失败。
![1-6.png](./img/1-6.png)